...
namespace RestClientApp
{
    class Program
    {
        ...
        {
            string baseUrl = "https://<localhost>/api/v1/companies/token";
        ...
        {
            string url = "https://<localhost>/api/v1/loadDocument/";
        ...
        public static async Task UploadFileAsync(string filePath, string bearerToken, string description)
        {
            ...
            {
                HttpClient client = new HttpClient();
                var url = $"https://<localhost>/api/v1/loadFile/upload?description={description}";
